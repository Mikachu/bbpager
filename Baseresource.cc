...
BaseResource::BaseResource(ToolWindow *toolwindow) {
 ...
 if (bbtool->image_dither && 
  ...
  if (XrmGetResource(resource_db, "bbpager.autoConfig",
                     "Bbpager.Autoconfig", &value_type, &value)) {
    if (! strncasecmp("true", value.addr, value.size)) {
      auto_config = True;
    } else
      auto_config = False;
  ...
  if (XrmGetResource(resource_db, "bbpager.autoConfig.checkTimeout",
                     "Bbpager.Autoconfig.CheckTimeout", &value_type, &value)) {
